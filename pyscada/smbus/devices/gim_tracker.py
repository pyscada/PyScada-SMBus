    ...
    def read_data(self, variable_instance):
        ...
        try:
            if variable_instance.smbusvariable.information == "tension":
                msb = (
                    self.inst.read_byte_data(self._device.smbusdevice.address, 0x1E)
                ) * 16
                lsb = (
                    self.inst.read_byte_data(self._device.smbusdevice.address, 0x1F)
                ) / 16
                return self.parse_value((msb + lsb) * 0.025)

            elif variable_instance.smbusvariable.information == "courant":
                msb = (
                    self.inst.read_byte_data(self._device.smbusdevice.address, 0x14)
                ) * 16
                lsb = (
                    self.inst.read_byte_data(self._device.smbusdevice.address, 0x15)
                ) / 16
                ...
                return self.parse_value(
                    (lsb + msb1 * 256 + msb2 * 65536 + msb3 * 16777216) * 0.0673157
                )
                ...
                return self.parse_value(
                    (msb2 * 65536 + msb1 * 256 + lsb) * 0.0673157 / 1000.0
                )
    ...
    def write_data(self, variable_id, value, task):
        ...
        if variable.writeable:
            if variable.smbusvariable.information == "raz":
                ...
                return self.parse_value(
                    self.inst.read_byte_data(self._device.smbusdevice.address, 0x01)
                )
        else:
